...
function Powerkey:init(x,y)
    -- x is placed in the middle
    self.x = x ---20 -- VIRTUAL_WIDTH / 2 - 32
...
function Powerkey:collides(target)
...
function Powerkey:render(quad)
    -- if powerup is in play draw ball
	
	if self.inPlay then	
		love.graphics.draw(gTextures['main'], gFrames['powerkey'][quad],
			self.x , self.y)
	end
end

-- wait solution from stackoverflow
