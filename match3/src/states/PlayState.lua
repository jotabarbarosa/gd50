--[[
	...
	RESET THE BOARD
	NOVO BOARD
	NOVOS TIJOLOS SELF.TILES
	
	
	
	
	TEMPO CONTINUA
	EVITAR CAIR COM MATCH
	RESET DURANTE PLAYSTATE
	
	
	
	ENTROU EM PLAY STATE
	TENTA FAZER TROCA
	
	SE A TROCA TEM MATCH
		CONTINUA E CONTA PONTO		
	SENAO
		CRIA NOVO BOARD
		VÊ SE TEM MATCH E RETIRA
...
function PlayState:enter(params)
	--print('entrou enter playstate')


	self.timer = params.tempo
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
			--print(#match)
			for m = 1, #match do
				if match[m].variety > 1 then 
					points = points + (match[m].variety * varietypoints)
					print(points)
				end
				if match[m].shiny then
					isShiny = true
					
				end					
			end
			--print('---------------')
            self.score = self.score + (#match * points)
...
function PlayState:render()
	...
	--print('vai renderizar board em playstate')
	self.board:render(self.level)
	...
	--love.graphics.circle('line', self.boardHighlightX * 32 + (VIRTUAL_WIDTH - 272)+16,
        --self.boardHighlightY * 32 + 32, 10)
    ...
    love.graphics.printf('Timer: ' .. tostring(self.timer), 20, 108, 182, 'center')
		...
		--love.graphics.draw(psystem, VIRTUAL_WIDTH/2, VIRTUAL_HEIGHT/2, 500)
end